...
class AdvSeg(UDADecorator):
    def __init__(self, **cfg):
        ...
        self.optimizer_D = {}
        for k in ["main", "aux"] if self.model.with_auxiliary_head else ["main"]:
            self.model_D[k] = FCDiscriminator(num_classes=self.num_classes)
            self.model_D[k].train()
            self.model_D[k].cuda()

            self.optimizer_D[k] = optim.Adam(self.model_D[k].parameters(), lr=self.lr_D, betas=(0.9, 0.99))
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img, target_img_metas, valid_pseudo_mask=None):
        ...
        if self.model.with_auxiliary_head:
            loss_aux = self.model.auxiliary_head.losses(pred["aux"], gt_semantic_seg)
        ...
        if isinstance(self.model, HRDAEncoderDecoder):
            for k in pred.keys():
                ...
                pred[k] = resize(
                    input=pred[k],
                    size=[int(e * self.model.feature_scale) for e in img.shape[2:]],
                    mode="bilinear",
        ...
        for k in pred_trg.keys():
            ...
            loss_G = self.loss_fn_D(D_out, Variable(torch.FloatTensor(D_out.data.size()).fill_(source_label)).cuda())
            # remember to have the word 'loss' in key
            g_trg_losses[f"G_trg.loss.{k}"] = self.lambda_adv_target[k] * loss_G
        ...
        if self.mic is not None:
            masked_loss = self.mic(
                self.get_model(), img, img_metas, gt_semantic_seg, target_img, target_img_metas, valid_pseudo_mask
            )
        ...
        for k in pred.keys():
            ...
            loss_D = self.loss_fn_D(
                D_out_src, Variable(torch.FloatTensor(D_out_src.data.size()).fill_(source_label)).cuda()
            )
        ...
        for k in pred_trg.keys():
            ...
            loss_D = self.loss_fn_D(
                D_out_trg, Variable(torch.FloatTensor(D_out_trg.data.size()).fill_(target_label)).cuda()
            )
        ...
        if self.local_iter % self.debug_img_interval == 0:
            ...
            for j in range(batch_size):
                ...
                subplotimg_daf(axs[0][1], torch.argmax(pred["main"][j], dim=0), "Source Seg", cmap="cityscapes")
                subplotimg_daf(axs[1][0], vis_trg_img[j], "Target Image")
                subplotimg_daf(axs[1][1], torch.argmax(pred_trg["main"][j], dim=0), "Target Seg", cmap="cityscapes")
                subplotimg_daf(axs[0][2], D_out_src[j], "Source Discriminator", vmin=0, vmax=1, cmap="viridis")
                subplotimg_daf(axs[1][2], D_out_trg[j], "Target Discriminator", vmin=0, vmax=1, cmap="viridis")
                for ax in axs.flat:
                    ax.axis("off")
