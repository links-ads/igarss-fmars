...
class MinEnt(UDADecorator):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img, target_img_metas, valid_pseudo_mask=None):
        ...
        for k in pred_trg.keys():
            ...
            trg_losses[f"ent.loss.{k}"] = self.lambda_ent[k] * entropy_loss(F.softmax(pred_trg[k], dim=1))
        ...
        if self.local_iter % self.debug_img_interval == 0:
            ...
            for j in range(batch_size):
                ...
                subplotimg(axs[1][1], torch.argmax(pred_trg["main"][j], dim=0), "Target Seg", cmap="cityscapes")
                vmin = torch.min(vis_ent[j]).item()
                vmax = torch.max(vis_ent[j]).item()
                subplotimg(axs[1][2], vis_ent[j], f"Target Ent {vmin:.2E}, {vmax:.2E}", cmap="viridis")
                for ax in axs.flat:
                    ax.axis("off")
                ...
                for j in range(batch_size):
                    ...
                    for k1, (n1, outs) in enumerate(seg_debug.items()):
                        for k2, (n2, out) in enumerate(outs.items()):
                            subplotimg(axs[k1][k2], **prepare_debug_out(f"{n1} {n2}", out[j], means, stds))
                    for ax in axs.flat:
                        ax.axis("off")
