    ...
    def __call__(
        ...
        if not self.source_only:
            ...
            # Use a separate EMA teacher for MIC
                ...
                masked_plabel, masked_pweight = self.teacher(target_img, target_img_metas, valid_pseudo_mask)
                if self.debug:
        ...
        # Use 1x source image and 1x target image for MIC
            ...
            masked_lbl = torch.stack([gt_semantic_seg[0], masked_plabel[0].unsqueeze(0)])
            gt_pixel_weight = torch.ones(masked_pweight[0].shape, device=dev)
            masked_seg_weight = torch.stack([gt_pixel_weight, masked_pweight[0]])
